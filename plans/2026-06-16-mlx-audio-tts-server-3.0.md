...
**For build verification and basic TTS pipeline testing**, use `mlx-community/Kokoro-82M-4bit` (~300MB download). This exercises model loading, request handling, PCM conversion, and the HTTP response path. Note: Kokoro does **not** support `ref_audio` voice cloning (it uses pre-loaded voice `.npz` vectors instead), so the `ref_audio` code path cannot be tested with this model.

**For `ref_audio` voice cloning testing**, use `mlx-community/chatterbox-turbo-fp16` (~3GB download). Chatterbox fully supports `ref_audio`/`ref_text` voice cloning. If download size is a concern on the test machine, the `ref_audio` path can be verified by code inspection and manual testing later on the Mac Mini with the 8-bit model.
...
5. **Raw PCM response**: The response body uses `ResponseBody` with explicit `contentLength` (the full audio is known after `generate()` returns). Audio is converted from `MLXArray` to `[Float]` via `.asArray(Float.self)`, then to raw bytes. Content-Type is `audio/pcm`.
...
- [x] 1. **Rewrite `Package.swift`** with correct dependencies and executable target
...
- [x] 2. **Move and rewrite the main source file** at `Sources/MLXAudioServer/main.swift`
...
- [x] 3. **Define the request/response types**
...
- [x] 4. **Implement the `/v1/audio/speech` POST handler**
...
- [x] 5. **Convert and stream the audio response**
...
- [x] 7. **Update tests** at `Tests/MLXAudioServerTests/MLXAudioServerTests.swift`
...
- [x] `swift build` compiles successfully with all dependencies resolved
- [x] Server starts with `MLXAudioServer --model mlx-community/Kokoro-82M-4bit` and prints model info + bind address to stderr
- [x] Server listens on port 8000 by default, or custom port via `--port`
- [x] `http POST localhost:8000/v1/audio/speech input="Hello world"` returns raw PCM audio bytes with Content-Type `audio/pcm`
- [x] `http POST localhost:8000/v1/audio/speech model=mlx-community/Kokoro-82M-4bit input="Hello world"` works (model matches)
- [x] `http POST localhost:8000/v1/audio/speech model=some-other-model input="Hello world"` returns HTTP 404 with JSON error
- [x] Missing `input` field returns HTTP 400 with JSON error
- [ ] `ref_audio` voice cloning path is tested with `mlx-community/chatterbox-turbo-fp16` (separate from basic pipeline testing with Kokoro)
- [x] Nonexistent `ref_audio` path returns HTTP 404 with JSON error
- [x] Output is playable via `play -q -t raw -r <sample_rate> -e floating-point -b 32 -c 1 -` (sample rate depends on model — Kokoro is 24000 Hz, Chatterbox is 24000 Hz)
...
7. **Kokoro requires a text processor**
   Mitigation: `TTS.loadModel` automatically provides `KokoroMultilingualProcessor` when loading Kokoro models (`TTSModel.swift:196-203`). No additional configuration needed.
